from tests.integration.credentials.recoverPassword import RecoverPasswordTestCase
from tests.integration.credentials.login import LoginTestCase
from tests.integration.credentials.logout import LogoutTestCase
from tests.integration.credentials.signup import SignUpTestCase
from tests.integration.credentials.confirmEmail import ConfirmEmailTestCase
from tests.integration.level.level import LevelTestCase
from tests.integration.level.levelType import LevelTypeTestCase
from tests.integration.message.errorMessage import ErrorMessageTestCase
from tests.integration.message.message import MessageTestCase
from tests.integration.user.rol import RolTestCase
from tests.integration.user.user import UserTestCase
from tests.integration.user.editSubjectsTestCase import EditSubjectsTestCase
from tests.integration.note.note import NoteTestCase



